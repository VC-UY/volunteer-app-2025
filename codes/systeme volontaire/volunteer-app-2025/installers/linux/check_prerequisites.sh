...
# Remonter de 2 niveaux: installers/linux/ -> installers/ -> volunteer-app-2025/
...
# Vérifier la configuration Redis dans backend/settings.py
if [ -f "$SCRIPT_DIR/backend/settings.py" ]; then
    echo -e "${GREEN}✓${NC} backend/settings.py: Présent"
    
    # Vérifier la configuration Redis
    REDIS_HOST=$(grep -oP "REDIS_PROXY_HOST\s*=\s*['\"]?\K[^'\"]*" "$SCRIPT_DIR/backend/settings.py" 2>/dev/null | head -1)
    REDIS_PORT=$(grep -oP "REDIS_PROXY_PORT\s*=\s*\K[0-9]+" "$SCRIPT_DIR/backend/settings.py" 2>/dev/null | head -1)
    ...
    if [ ! -z "$REDIS_HOST" ] && [ ! -z "$REDIS_PORT" ]; then
        ...
        echo -e "  ${YELLOW}⚠${NC} Impossible de lire la configuration Redis (valeurs par défaut utilisées)"
        ((WARNINGS++))
    fi
else
    echo -e "${RED}✗${NC} backend/settings.py: Manquant"
    ((ERRORS++))
fi
