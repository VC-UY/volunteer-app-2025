...
def _run_auth_volunteer_flow_background():
    """Exécute auth_volunteer_flow() en arrière-plan.

    Le proxy Redis du coordinateur peut rester muet indéfiniment sur une
    commande (ex: PUBLISH) sans jamais lever d'erreur socket ni de timeout
    (observé en pratique : connexion TCP établie mais aucune réponse, y
    compris avec redis-cli en dehors de toute lib Python). Comme
    auth_volunteer_flow() est normalement appelée depuis AppConfig.ready(),
    un tel silence bloquerait indéfiniment le démarrage de Daphne et donc
    l'affichage du dashboard local. On l'isole donc dans un thread dédié.
    """
    try:
        auth_volunteer_flow()
    except Exception as e:
        logger.error(f"Erreur lors de l'authentification/enregistrement du volontaire: {e}")
        import traceback
        logger.error(traceback.format_exc())
...
class RedisAppConfig(AppConfig):
    ...
    def ready(self):
        ...
        try:
            ...
            # Lancée en arrière-plan : un coordinateur lent/muet ne doit jamais bloquer
            # le démarrage de Daphne (voir _run_auth_volunteer_flow_background).
            threading.Thread(
                target=_run_auth_volunteer_flow_background,
                daemon=True,
                name="auth-volunteer-flow",
            ).start()
