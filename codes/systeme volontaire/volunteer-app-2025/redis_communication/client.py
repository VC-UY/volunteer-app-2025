...
class RedisClient:
    ...
    def _initialize_connection(self):
        ...
        try:
            ...
            # le proxy coordinateur mélange mal les deux, cf. connexions qui restent bloquées
            # indéfiniment sur une commande alors que le SUBSCRIBE fonctionne)
    ...
    def publish(self, channel: str, message_data: Any, request_id: str = None, token: str = None, message_type: str = None):
        ...
        # Publier le message via une connexion dédiée, à durée de vie courte et
        # timeout court (3s) : le proxy coordinateur peut rester silencieux
        # indéfiniment sur un PUBLISH envoyé sur la connexion principale
        # (observé : connexion établie, aucune réponse, même avec redis-cli en
        # dehors de toute lib Python) sans jamais déclencher le socket_timeout
        # normal — utiliser une connexion à part, avec un timeout court, évite
        # qu'un tel silence bloque tout le reste (heartbeat, écoute des
        # tâches...), qui partageraient sinon la même connexion "self.redis".
        try:
            ...
            # Ne plus marquer self.connected = False ici : un PUBLISH lent/en
            # échec ne signifie pas que la connexion d'écoute (pubsub) est
            # morte — la marquer déconnectée déclenchait une boucle de
            # reconnexion incessante (watchdog) alors que l'abonnement
            # fonctionnait très bien.
            logger.warning(f"Publication {channel} ignorée (coordinateur lent/hors ligne): {e}")
    ...
    def _reconnect(self):
        ...
        try:
            ...
            # Reprise immédiate : heartbeat + écoute des assignations, sans
            # attendre le prochain tick du watchdog.
            try:
    ...
    def _watchdog_loop(self):
        ...
        while self.running:
            ...
            if not self.connected:
                ...
                # Ne pas PING sur la connexion pubsub (le proxy coordinateur
                # gère mal le mélange PING/SUBSCRIBE sur le même socket, cf.
                # _initialize_connection). On se base sur l'activité récente
                # (messages reçus/publiés) plutôt que sur un ping actif.
