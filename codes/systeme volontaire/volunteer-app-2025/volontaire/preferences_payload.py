...
def task_matches_preferences(task_data: dict, prefs: dict | None) -> tuple[bool, str]:
    """Filtre une tâche assignée selon les capacités réelles du runtime vc-uyr.

    Le runtime ne tient pas compte des créneaux horaires (is_available_now) :
    on ne filtre plus sur la disponibilité configurée, seulement sur les
    ressources et types de calcul, qui correspondent à ce que le runtime
    peut effectivement exécuter (POST /api/resources).
    """
