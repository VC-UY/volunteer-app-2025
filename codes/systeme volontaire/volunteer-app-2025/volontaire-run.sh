...
install_redhat() {
    ...
    echo -e "${YELLOW}[2/5] Installation de Redis...${NC}"
    if ! command_exists redis-server; then
        yum install -y redis
    ...
    echo -e "${YELLOW}[3/5] Installation de Git et autres outils...${NC}"
    yum install -y git curl wget tmux

    echo -e "${YELLOW}[4/5] Installation de Node.js et npm...${NC}"
    if ! command_exists node; then
