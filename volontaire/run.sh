...
chmod +x ./install_runtime.sh 2>/dev/null || true
./install_runtime.sh >/dev/null 2>&1 || true

if ! curl -sf --max-time 2 "$RUNTIME_URL/api/health" >/dev/null 2>&1; then
    echo "✅ Démarrage runtime (foreground debug)..."
    if [[ "${FORCE_COMPAT:-0}" != "1" && -x "$RUNTIME_BIN" && -f "$RUNTIME_CFG" ]]; then
        nohup python runtime_auth_shim.py --port "$AUTH_SHIM_PORT" \
            >>.volunteer/logs/runtime-auth-shim.log 2>&1 &
        if command -v sudo >/dev/null && sudo -n true 2>/dev/null; then
            nohup sudo -n env RUST_LOG=info "$RUNTIME_BIN" "$RUNTIME_CFG" \
                >>.volunteer/logs/runtime.log 2>&1 &
        else
            nohup env RUST_LOG=info "$RUNTIME_BIN" "$RUNTIME_CFG" \
                >>.volunteer/logs/runtime.log 2>&1 &
        fi
    ...
    if ! curl -sf --max-time 1 "$RUNTIME_URL/api/health" >/dev/null 2>&1; then
